...
sbit    EOC=P3^2;               //A/D结束状态线，逻辑1为忙于转换
...
{ T0T1_init();                  //T0、T1初始化程序(11.0592MHz)
  ...
  { if(new_cycle_flag==1)       //有新数据,Vs1=-(R5/R4)VT8=-(20K/100K)VT8=-0.2VT8
    { 
	  ...
	  //经过调理电路采集，所以根据调理电路公式计算出实际采集电压大小，利于数码管显示
...
{ TH0=0xfb;                     //T0时间常数高字节重装
  ...
  if(M==8)                      //10ms到了吗？因为1.25ms*8=10ms          采样周期10ms
  { AD0=0x00;                   //到了10ms，启动0通道AD转换
    M=0;                        //M清0
    new_cycle_flag=1;           //置新采样数据标志
    while(EOC==1);              //若P3.2还是逻辑1，继续查
    r_kT=AD0;                   //读取AD结果
  }
  B_port=0xff;                  //关闭显示
